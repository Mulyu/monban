...
| 2 | `forbidden` | ファイル内の禁止テキストパターン・BOM・不可視文字・シークレット・プロンプトインジェクション・マージコンフリクトマーカー・CRLF 改行コードを検出する |
| 3 | `size` | ファイルの行数を上限／下限の範囲内に収めるか検証する |
...
`pattern`、`json_key`、`bom`、`invisible`、`secret`、`injection`、`conflict`、`crlf` のいずれか 1 つ以上を指定する。`json_key` はバイト列検査 (`bom`/`invisible`/`secret`/`injection`/`conflict`/`crlf`) とは併用不可。
...
| `crlf` | boolean | No* | — | `true` で CRLF 改行コードを検出する（`\r\n` 行混在や CRLF のみのファイル） |
| `message` | string | No | — | エラーメッセージ |
| `severity` | `"error"` \| `"warn"` | No | `"error"` | 重大度 |

\* `pattern`、`json_key`、`bom`、`invisible`、`secret`、`injection`、`conflict`、`crlf` のいずれか 1 つ以上が必須。`json_key` はバイト列検査 (`bom`/`invisible`/`secret`/`injection`/`conflict`/`crlf`) と併用不可。
...
`\n` で分割した各行のうち、末尾が `\r` で終わるものを違反として報告する。Windows 環境で編集された shell スクリプト・Dockerfile・YAML が紛れ込んだケースを検出する。monban は検出のみで自動整形はしない（エディタや `dos2unix` に任せる）。
...
ファイルの行数（line count）を上限／下限の範囲に収めるか検証する。AI エージェントは 1 ファイルに機能を詰め込みがち（`max_lines` で上限）、そして生成プレースホルダや空 stub を残しがち（`min_lines` で下限）。
...
content:
  size:
    ...
    # 下限 — プレースホルダや空 stub を検出する
    - path: "docs/**/*.md"
      min_lines: 5
      message: "ドキュメントが短すぎます。"
```
...
| `path` | string | Yes | — | 対象ファイルの glob パターン |
| `exclude` | string[] | No | `[]` | 対象から除外する glob パターン |
| `max_lines` | integer | No\* | — | 許容する最大行数（この値を超えると違反） |
| `min_lines` | integer | No\* | — | 必要な最小行数（この値を下回ると違反） |
| `message` | string | No | — | エラーメッセージ |
| `severity` | `"error"` \| `"warn"` | No | `"error"` | 重大度 |

\* `max_lines` か `min_lines` のいずれか 1 つ以上が必須。両方指定する場合は `min_lines` が `max_lines` を超えてはならない。

### 判定方法

1. 対象ファイルを読み込む
2. 行数をカウント（末尾の空行は除外）
3. `max_lines` が設定されていて行数がそれを超えていれば違反として報告
4. `min_lines` が設定されていて行数がそれを下回っていれば違反として報告
