...
monban は、こうした「全体の一貫性が崩れる」現象を CI やローカルで検出する番人です。PR レビューで新規違反だけを叩きたい場合は、全コマンドに共通する `--diff` フラグで差分スコープに限定できます（[diff.md](diff.ja.md)）。
...
- **組織横断の再利用を `extends` で支援** — base ルールを GitHub から継承可能（[extends.md](extends.ja.md)）
