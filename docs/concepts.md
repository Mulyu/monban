...
| **monban** | **プロジェクト構造・ファイル内容・ドキュメント整合性・依存名の実在** |
...
- **外部ネットワーク依存は `monban deps` に局在化** — 依存パッケージのレジストリ照合のみが外部 API を使う。他のコマンドは完全ローカルで動作する。オフライン環境では `deps --offline` で allowlist / denylist のみ実行する
