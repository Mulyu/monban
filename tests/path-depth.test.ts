...
describe("path/depth", () => {
	...
	it("passes when within limits", async () => {
		const results = await checkDepth([{ path: "src", max: 10 }], cwd);
		expect(results).toHaveLength(0);
	});

	it("counts depth relative to base path", async () => {
		const results = await checkDepth([{ path: "src/domain", max: 2 }], cwd);
