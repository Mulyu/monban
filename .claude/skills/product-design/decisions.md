...
## 2026-04-19: `fail_text` / `docs_url` 共通フィールド案を撤回

2026-04-19 の決定で「横断メタ機能の優先度 1」として採用していた **`fail_text` + remediation JSON** を撤回する。実装途中（PR #52）で「`message` と機能が被っている」という観点からレビューし、再評価した結果。

### 撤回理由

#### ① `message` と役割が被る

設計上の意図は `message = WHAT` / `fail_text = HOW` の分離だった:

```yaml
message: "domain 層で process.env を使わない"        # 何が悪いか
fail_text: "config を application 層で注入してください"  # どう直すか
```

しかし実用上、ユーザは両方に書く動機が薄く、結局 `message` に両方を書く:

```yaml
message: "doman 層で process.env を使わない。config を application 層で注入してください。"
```

`fail_text` を使う動機が見当たらない。

#### ② 構造化の便益が「理論上のもの」に留まる

JSON 出力で `remediation: { fail_text, docs_url }` に分離する利点は **AI エージェントがフィールド単位で消費する** ことだったが、現状そのような挙動を実装している coding agent は確認できていない。lefthook の `fail_text` precedent は **コマンド runner の失敗時メッセージ** で、設定 linter の monban とは文脈が異なる。

#### ③ monban の哲学に反する

monban の設計原則「**重複の排除**: 同じことを別の書き方で表現できるルールは統合する」に直接抵触する。`message` で十分覆える情報を別フィールドに切り出す合理性がない。

### 不採用 — `fail_text`

| 候補 | 理由 |
|---|---|
| `fail_text` を全ルール共通フィールドとして追加 | `message` と機能が被る。重複排除原則に抵触 |
| `docs_url` を全ルール共通フィールドとして追加 | `fail_text` 不採用に伴いペアで撤回。必要なら `message` 内に URL を含めれば足りる |
| JSON 出力の `remediation: { ... }` フィールド | 上記 2 つの不採用に伴い撤回 |

### 残された「横断メタ機能」の優先度

撤回後の優先度（2026-04-19 改訂）:

1. ~~`fail_text` / remediation~~ — **撤回**
2. **終了コード規律** — PR #47 で実装済み
3. ~~`--sarif` 出力~~ — 不採用（範囲外、user 方針）
4. ~~severity / baseline / tags~~ — 不採用（範囲外、user 方針）

横断メタ機能で残るのは終了コード規律のみ。`monban agent`（残り実装順 #6）が次の作業で、ここで実装計画は完了する。

### 撤回ステータス

PR #52 (`feat(remediation): add fail_text and docs_url to all rules`) はマージせずクローズ。実装は破棄。
