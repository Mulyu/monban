...
| `monban license` | LICENSE ファイルのライセンス判別（SPDX タグ / テンプレート照合）+ ソースヘッダの SPDX-License-Identifier 検査 | 採用（前判断の限定的転換） | `path.required` + `content.required` で同等は書けるが、「ライセンス設定」という単一名前空間で表現できる UX を優先。SBOM / SLSA 署名検証 / 依存ライセンス検査は引き続き範囲外（cargo-deny / licensee の領域）。前判断（「ライセンスコンプライアンス全般」「LICENSE の存在のみ」）を、SPDX 識別子の判別と allowlist 検査に限定して採用 |
| `monban docker` | Dockerfile の `pinned` / `user` / `healthcheck` / `forbidden`（命令の denylist + 引数 regex） | 採用（前判断の限定的転換） | `content.forbidden` の正規表現で同等は書けるが、Dockerfile 専用の語彙（FROM ピン、USER root 禁止、HEALTHCHECK 必須、ADD URL 禁止）の方が UX 上自明。Dockerfile を単純な行ベースで解析するだけで AST 不要なので言語非依存原則も保てる。深い検査（マルチステージ依存解析等）は引き続き hadolint の領域、`compose` / k8s manifest は範囲外 |
...
| `runtime.consistency.preset` | `node` / `python` / `ruby` / `go` / `rust` の組み込み source 群を 1 行で展開。user の `sources` と併用時は **連結**（preset の sources に追記） | 採用 | 既存 `git.commit.message.preset: conventional` と同形式で学習コストゼロ。連結セマンティクスで「preset で広く拾い、プロジェクト固有 source を追記」が容易 |
| preset の `sources` 上書き semantics | preset の sources を user の sources で置換 | 不採用 | preset を使う動機（書き直し回避）を打ち消す。連結が自然 |
| `presets: [node, python]` の配列 shorthand | 1 ルール = 1 言語と崩れる | 不採用 | `[{preset: node}, {preset: python}]` と等価で重複排除違反 |
| preset カタログの YAML 外部ファイル化 | カスタム preset を yaml で追加可能に | 不採用 | 5 言語までは TS const で十分。需要顕在化まで保留 |
| preset への `engines.node` / `requires-python` 含有 | `package.json` / `pyproject.toml` の range 表記場所も preset に含める | 不採用 | `>=20` や `^20.0.0` は厳密一致を崩して誤検出になる。必要なら user が個別追加 |
...
| `npm run monban` スクリプト追加 | `package.json` に `node dist/bin.mjs` ラッパを追加 | 採用 | 自リポ内では `npx` を使わず固定エントリで dogfood する。配布物（README）は引き続き `npx` を案内 |
| `.claude/hooks/guard-bash.sh`（PreToolUse:Bash） | `npx` / `git push --force` to main / `--no-verify` / `git config` 書換を deny | 採用 | リポジトリ自身の事故防止ガード。配布スコープ外（プラグインに同梱しない） |
| `.claude/hooks/self-check.sh`（Stop） | `npm run monban -- all --diff=$BASE` を実行し違反を exit 2 で Claude に流す | 採用 | PostToolUse 自動実行（不採用済み）と差別化: 毎編集ではなくターン終了 1 回、ブロックではなく Claude へ報告して次ターンで修正させる。"検出のみ、修正はしない" 哲学を保つ |
| `self-review` スキル | direction 原則 + decisions.md 不採用案との自己整合チェック専用 | 採用 | 汎用品質は simplify、PR 総合は review、セキュリティは security-review に委譲。monban 固有の判断軸でのレビュー枠が空いていた |
| `agent.subagents` | `.claude/agents/*.md` の frontmatter 必須キー（`name` / `description`）と `model` allowlist | 採用 | 既存 `agent.instructions` と同じ「エージェント設定ファイル品質担保」枠。判定手段（YAML frontmatter + 本文走査）も同じなので独立コマンド化はせず `agent` 配下のルールとして追加 |
| `agent.skills` | `.claude/skills/*/SKILL.md` の frontmatter 必須キー（`name` / `description`）と description 長上限 | 採用 | `agent.subagents` と同枠。skill は短い `description` で発火するため、長さ上限フィールドを別途持つ点だけが subagent と異なる |
| `git.commit.author` | author email の `allowed` / `forbidden`（`noreply` 必須・社内ドメイン縛り・誤 author 検出） | 採用 | `branch_name` / `tag_name` と並列。Git メタデータ層に commit author 軸が欠落していた。判定は `git log --format=%ae` で完結し外部 API 不要 |
| `agent.settings.hooks.timeout` | `.claude/settings.json` の全 hook に `timeout` 設定必須 | 採用 | `github.actions.timeout` と同発想（必須化による暴走抑止）。`agent.settings` のフィールド拡張として追加し独立ルール化はしない |
| `runtime.consistency.preset` 拡張 | `java` / `dotnet` / `php` / `kotlin` を追加（既存の node/python/ruby/go/rust に追記） | 採用 | 前判断の「5 言語までは TS const で十分。需要顕在化まで保留」を、需要顕在化に応じて段階的に解除。preset 外部ファイル化（不採用）の方針は維持 |
| `content.size.min` | 行数下限（glob + 行カウント） | 採用 | 既存 `content.size` の上限と対称。プレースホルダ／空ファイル／生成失敗検出。`max` と同フィールドペアで `forbidden` / `required` の語彙には流さず数値範囲として扱う |
| `content.forbidden.crlf` プリセットフラグ | CRLF 改行コード検出 | 採用 | `bom` / `invisible` / `conflict` と同列のプリセット拡張。`editorconfig` と領域は重なるが monban は「検出」のみに留め、自動整形は行わない |
