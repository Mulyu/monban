...
| `agent.ignore.must_cover` → `required` | 「含まれていなければ違反」= required セマンティクス。語彙統一のため改名 |
| `deps.install_scripts.hooks` → `forbidden` | 「このフックが宣言されていたら違反」= forbidden セマンティクス |
| `agent.instructions.frontmatter_keys` → `allowed_frontmatter_keys` | 実態は allowlist。語彙を明示 |

### allowed / forbidden ペアの対称化

| 変更 | 内容 |
|---|---|
| `github.actions.runner.forbidden` 追加 | `self-hosted` 単独禁止を簡潔に書けるよう `allowed` と対称化 |
| `github.actions.secrets.forbidden` 追加 | 退役シークレットの明示禁止（`allowed` と対称化） |
| `git.branch_name.forbidden` 追加 | `wip/` `tmp/` 等を regex denylist で禁止可能に（`pattern` は引き続き allowlist 的） |
| `git.tag_name.allowed` / `forbidden` 追加 | 既存非準拠タグの個別免除、beta/rc 等の禁止を表現。`branch_name` と API 対称 |
| `deps.floating_version.allowed` 追加 | 社内 `@myorg/*` のみ `^` を許す等の例外表現 |
| `github.actions.forbidden.uses` 配列許容 | 1 ルールで複数禁止アクションを列挙可能に |

### 配布形態

| 変更 | 内容 |
|---|---|
| Claude Code プラグイン化 | `plugins/monban/` に skill + `/monban:init` を同梱。マーケットプレイス `.claude-plugin/marketplace.json` を同一リポジトリに配置し、CLI のバージョンと release-please で同期（`extra-files`） |
| スキルは単一 `monban` | コマンド別分割は direction の「重複排除」に反する。全体マップ + docs リンクに集約 |
...
| 候補 | 理由 |
|---|---|
| 別リポジトリ `Mulyu/monban-marketplace` | CLI とプラグインの version drift リスク。同梱 + release-please `extra-files` で同期する方が堅牢 |
| コマンド別スキル分割（`monban-path` 等 7 個） | 「重複排除」違反。エージェントが欲しいのは全体マップ + docs リンク |
| PostToolUse フックで Edit/Write 後に自動 `monban all` | 侵襲的。monban は CI + ローカル明示実行が前提。毎編集コストが高い。README に optional レシピで留める |
| `/monban:check` スラッシュコマンド | `npx @mulyu/monban all` の単純ラップで価値ゼロ |
| `/monban:fix` スラッシュコマンド | スキル本文「修正ワークフロー」と重複 |
| monban の MCP server 化 | 用途が違う（外部呼び出し用）。エージェント内ループでは Bash で十分 |
| `thinking` / `direction` を同一プラグインに同梱 | monban CLI と独立した汎用スキル。混ぜると「monban 入れたら無関係スキルも増える」UX になる。将来 `mulyu` マーケットプレイスの別プラグインとして切り出す |
